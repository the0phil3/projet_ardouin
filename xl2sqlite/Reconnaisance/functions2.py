...
def Xlopen(sheetname):
    sheetname = pd.read_excel('~/dev/projet_ardouin/sources/sources_excel/Ardouin La Totale.xls', sheetname, header=0)
...
def Master_concat(*dataframes):
    ...
    final = final.drop_duplicates(subset=['unittitle', 'function'], keep='first')
    return final
